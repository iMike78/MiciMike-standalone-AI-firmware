...
#ifndef API_INPUT_CHANNEL
#define API_INPUT_CHANNEL 0
#endif

static inline int16_t api_channel_frame_to_mono_16(const int32_t *in, size_t frame)
{
    const size_t channel = API_INPUT_CHANNEL > 1 ? 0 : API_INPUT_CHANNEL;
    return (int16_t)(in[frame * 2 + channel] >> 16);
}

size_t audio_resample_to_api(const int32_t *in, size_t in_frames,
                             int16_t *out, size_t out_size)
{
    // Step 1: Select one XMOS output channel and convert 32->16 bit.
    ...
    for (size_t i = 0; i + 1 < in_frames && out_idx + 3 <= max_out; i += 2) {
        int16_t s0 = api_channel_frame_to_mono_16(in, i);
        int16_t s1 = api_channel_frame_to_mono_16(in, i + 1);

        int16_t s2;
        if (i + 2 < in_frames) {
            s2 = api_channel_frame_to_mono_16(in, i + 2);
        } else {
            s2 = s1;
        }
                                 ...
                                 int32_t *out, size_t out_size)
{
    ...
    size_t max_frames = out_size / (2 * sizeof(int32_t));
    size_t out_frames = 0;

    for (size_t i = 0; i < in_samples && out_frames + 2 <= max_frames; i++) {
        int32_t s0 = (int32_t)in[i] << 16;
